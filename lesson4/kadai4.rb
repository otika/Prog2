...
class BookInfoManager
	...
	def search
		begin
			...
			query_foot = ";"
			query_body = ""
			query_body += (key!="")? "id=\'#{key}\' and ": ""
			query_body += (title!="")? "title=\'#{title}\' and " : ""
			query_body += (author!="")? "author=\'#{author}\' and " : ""
			query_body += (page!=0)? "page=#{page} and " : ""
			query_body += "publish_date like \'"
			query_body += (year!=nil) ? sprintf("%02d-",year) : "%-"
			query_body += (month!=nil) ? sprintf("%02d-",month)  : "%-"
			query_body += (day!=nil) ? sprintf("%02d",day)  : "%"
			query_body += "\'"
